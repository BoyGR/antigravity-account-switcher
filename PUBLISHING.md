...
- **VSIX File**: `release/antigravity-account-switcher-0.8.0.vsix`
...
4. Di dashboard publisher `boygr`:
   ...
   - Drag & drop atau pilih file:
     ```
     release/antigravity-account-switcher-0.8.0.vsix
     ```
   - Klik **Upload**.
...
```powershell
npx @vscode/vsce publish --packagePath release/antigravity-account-switcher-0.8.0.vsix
```
...
4. Klik **Publish Extension**:
   - Unggah file `release/antigravity-account-switcher-0.8.0.vsix`.
...
```powershell
npx ovsx publish release/antigravity-account-switcher-0.8.0.vsix -p <OPEN_VSX_TOKEN>
```
Atau login:
```powershell
